...
- **HealthTech 6-pack** — six sibling specs / profiles / labs that fan out the Suite's regulated-vertical coverage from Clinical AI Disclosure into FHIR access auditing, FDA SaMD lifecycle records, HIPAA Security Rule readiness bundles, PHI vault contracts, clinical bias-cohort coverage, and the medical adverse-event Incident Card profile. All v0.1 draft, all MIT (spec-side licensing), all `kinetic-gain-protocol-suite` + `healthtech` topic-tagged, all composing with each other via `linked_records` so a single deployment's evidence reads as one graph:
...
Current public GitHub count: **406 repos**. Operator-surface hardening backlog (squad doctrine v1.1): **49 .kineticgain.com subdomains** now at `v1.0-prod`, every Codex-shipped v0.1 caught up — zero gaps remaining at the cutoff. The full grouped index is at [kineticgain.com/constellation](https://kineticgain.com/constellation/). Constellation security posture: **30 / 30 buyer-facing surfaces at A/90** on the kg-header-audit rubric — HSTS preload-ready, CSP locked, fonts self-hosted, COEP `require-corp` enabled.
